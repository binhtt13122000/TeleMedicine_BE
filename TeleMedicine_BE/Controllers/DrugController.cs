...
namespace TeleMedicine_BE.Controllers
{
    ...
    public class DrugController : ControllerBase
    {
            ...
            try
            {
                ...
                if (!String.IsNullOrEmpty(filters))
                {
                    ...
                    if (checkHasProperty)
                    {
                        ...
                        string json = jsonIgnore.JsonIgnore(typeof(DrugVM), splitFilter, paged, PropertyRenameAndIgnoreSerializerContractResolver.IgnoreMode.EXCEPT);
        ...
        [HttpPut]
        [Produces("application/json")]
        public async Task<ActionResult<DrugVM>> UpdateDrug([FromBody] DrugUM model)
        {
            DrugType drugType = await _drugTypeService.GetByIdAsync(model.DrugTypeId);
            if (drugType == null)
            {
